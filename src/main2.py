...
def mark_attendance(class_name):
    ...
    detection_model = load_caffe_model(
        r'C:\Users\laptop zone\SmartAttend\detection_model\architecture.prototxt',
            ...
            for detection in detections:
                ...
                if x1 < x2 and y1 < y2:
                    ...
                    if crop.size > 0:
                        ...
                        input_data = np.array(crop).reshape(1, -1)
                        ...
                        if text != 'Unknown' and text not in detected_students:
                            detected_students.append(text)

                        output_frame = cv2.rectangle(output_frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                        cv2.putText(output_frame, text, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

        # Show frame
        cv2.imshow('Real-Time Attendance', output_frame)
